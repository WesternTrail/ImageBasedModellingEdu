...
void LocalViewSelection::performVS(){
    ...
    /*获取参考视角*/
    SingleView::Ptr refV = views[settings.refViewNr]; // 取出第二个帧
    ...
    math::Vec3f refDir = (p - refV->camPos).normalized(); // 获得到链接patch中心相机光心的射线

    /* 计算patch在参考视角和其他所有视角的的ncc, 极平面的法向量 */
    std::map<std::size_t, math::Vec3f> viewDir; // 当前视角的射线方向。
    std::map<std::size_t, math::Vec3f> epipolarPlane; //patch的法向量 // plane normal
    std::map<std::size_t, float> ncc;
    for (std::size_t i = 0; i < views.size(); ++i) { // 遍历所有视图与参考视图，计算NCC和级平面法向量
        if (!available[i]) // 如果当前视图不是全局选择的视角
            continue;
        ...
        /*计算极平面之间的法向量*/
        viewDir[i] = (p - views[i]->camPos).normalized(); // 连接3维点到当前视角的射线
        epipolarPlane[i] = (viewDir[i].cross(refDir)).normalized(); // 计算垂直于极平面的法向量，用来判断是否两个视角相似度太高？
    }
    ...
    bool foundOne = true;
    while (selected.size() < settings.nrReconNeighbors && foundOne){ // 找到4个局部视角
        foundOne = false;
        std::size_t maxView = 0;
        float maxScore = 0.f;
        for (std::size_t i = 0; i < views.size(); ++i) {
            if (!available[i]) // 如果ncc小于我们设定的阈值，这里的available[i]是false
                continue;
            float score = ncc[i];
            ...
            if (mfp / nfp < 0.5f) {  //第i个视角的分辨率小于参考视角的分辨率，则score减小
                score *= 0.01f;
            }
            ...
            for (sel = selected.begin(); sel != selected.end(); ++sel) {
                ...
                dp = math::clamp(viewDir[*sel].dot(viewDir[i]), -1.f, 1.f); // 当前视角和已选视角的射线的夹角
