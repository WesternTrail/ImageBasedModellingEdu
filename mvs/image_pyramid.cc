...
namespace
{
    ...
    ImagePyramid::Ptr
    buildPyramid(core::View::Ptr view, std::string embeddingName)
    {
        ImagePyramid::Ptr pyramid(new ImagePyramid());
        ImagePyramid& levels = *pyramid;

        core::View::ImageProxy const* proxy = view->get_image_proxy(embeddingName); // 读取name为undistored的图片信息
        core::CameraInfo cam = view->get_camera();

        assert(proxy != nullptr);

        int curr_width = proxy->width; // 初始的图片像素大小
        int curr_height = proxy->height;

        levels.push_back(ImagePyramidLevel(cam, curr_width, curr_height)); // 首先

        while (std::min(curr_width, curr_height) >= MIN_IMAGE_DIM) // 把图像的宽高每次缩减一半加入到levels中
        {
            if (curr_width % 2 == 1)
    ...
    void
    ensureImages(ImagePyramid& levels, core::View::Ptr view,
        std::string embeddingName, int minLevel)
    {
        if (levels[minLevel].image != nullptr)
            return;

        core::ByteImage::Ptr img = view->get_byte_image(embeddingName); // 获取到对应的图像
        int channels = img->channels();

        /* Remove alpha channel. */
        if (channels == 2 || channels == 4)
            core::image::reduce_alpha<uint8_t>(img);
        /* Expand grayscale images to RGB. */
        if (img->channels() == 1)
            img = core::image::expand_grayscale<uint8_t>(img);
        ...
        for (int i = 0; std::min(curr_width, curr_height) >= MIN_IMAGE_DIM; ++i)
        {
            ...
            if (i > 0)
            {
                img = core::image::rescale_half_size_gaussian<uint8_t>(img, 1.f); // 高斯resize不断减少一半尺寸
...
ImagePyramidCache::get(core::Scene::Ptr scene, core::View::Ptr view,
    std::string embeddingName, int minLevel)
{
    ...
    if (scene != ImagePyramidCache::cachedScene // 如果当前场景不在cachedScene中
...
core::Scene::Ptr ImagePyramidCache::cachedScene;
