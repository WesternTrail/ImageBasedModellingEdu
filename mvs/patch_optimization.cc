    ...
    localVS(views, settings, _globalViewIDs, _localViewIDs, sampler)
{
    ...
    /** 对每个点进行局部视角选择 **/
    localVS.performVS();
    if (!localVS.success) {  //并没有选择到合适的局部视角，则直接返回
        status.optiSuccess = false;
        return;
    }
    ...
    /** 计算每个视角的颜色尺度 **/
    computeColorScale(); // 说实话不知道这个是干啥的？
}

void PatchOptimization::computeColorScale(){

    if (!settings.useColorScale) // 不使用颜色尺度？
        return;
    ...
    for (id = neighIDs.begin(); id != neighIDs.end(); ++id) {
        ...
        for (int c = 0; c < 3; ++c) {
            float ab = 0.f;
            float aa = 0.f;
            for (std::size_t i = 0; i < mCol.size(); ++i) { // 遍历patch中的25个点
...
void PatchOptimization::doAutoOptimization(){
    ...
    while (status.iterationCount < settings.maxIterations &&
        ...
        for (id = neighIDs.begin(); id != neighIDs.end(); ++id, ++count) {
            ...
            // ncc的值小于可接受的范围或者优化次数达到一定值但仍然尚未收敛，则移除视角
...
void PatchOptimization::optimizeDepthOnly(){
    ...
    for (id = neighIDs.begin(); id != neighIDs.end(); ++id){
        ...
        // 计算邻域视角采样点的颜色，和颜色的偏导
        Samples nCol, nDeriv;
        sampler->fastColAndDeriv(*id, nCol, nDeriv); // 计算偏导？
        if (!sampler->success[*id]) {
            status.optiSuccess = false;
            return;
        }
    ...
    if (denom > 0) {
        depth += numerator / denom; // 不知道为啥颜色的要加在深度上？
