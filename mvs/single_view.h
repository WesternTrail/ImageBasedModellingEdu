...
SingleView::worldToScreenScaled(math::Vec3f const& point)
{
    ...
    math::Vec3f cp(this->worldToCam.mult(point,1.f)); // 世界坐标系->相机坐标系
    math::Vec3f sp = this->target_level.proj * cp; // 相机坐标系->像素坐标系？

    math::Vec2f res(sp[0] / sp[2] - 0.5f, sp[1] / sp[2] - 0.5f);    // 减去0.5其实没太大影响可能是获得中心点
    return res;
}
