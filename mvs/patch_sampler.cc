...
PatchSampler::PatchSampler(std::vector<SingleView::Ptr> const& _views,
    ...
    masterColorSamples.resize(nrSamples); // patch 的三维点在参考视角中的颜色
    ...
    math::Vec2i h;
    h[0] = h[1] = offset;
    topLeft = midPix - h;       // 在参考图像上的的patch左上角
    bottomRight = midPix + h;   // 在参考图像上的patch的右下角
    ...
    /**利用patch几何模型的公式d(i,j) = d + i * hs(s,t) + j* ht(s,t) 计算每个点的深度,同时获得每个patch点的三维坐标 **/
    computePatchPoints();
}
...
void PatchSampler::fastColAndDeriv(std::size_t v, Samples& color, Samples& deriv){
    ...
    mmLevel = views[v]->clampLevel(mmLevel); // 限制采样的倍数处于正确范围

    /* 计算偏导的计算步长？ */
    math::Vec3f p1(p0 + masterViewDirs[nrSamples/2]); // 这个是啥？
    float d = (views[v]->worldToScreen(p1, mmLevel) - views[v]->worldToScreen(patchPoints[12], mmLevel)).norm();
    if (!(d > 0.f)) {
        return;
    }
    stepSize[v] = 1.f / d;  //每stepsize取一个采样点？
    ...
    /* 对于参考视角每个sample点在当前视角v的梯度方向 */
    std::vector<math::Vec2f> gradDir(nrSamples); // todo 看不懂
    for (std::size_t i = 0; i < nrSamples; ++i){
        math::Vec3f p0(patchPoints[i]);
        math::Vec3f p1(patchPoints[i] + masterViewDirs[i] * stepSize[v]); // p0,p1好像差距不大
...
float PatchSampler::getFastNCC(std::size_t v){
    ...
    float sqrDevY = 0.f;
    float devXY = 0.f;
    for (std::size_t i = 0; i < nrSamples; ++i){ // 可能是NCC的计算公式
    ...
    float tmp = sqrt(sqrDevX * sqrDevY); // ncc 分母
...
math::Vec3f PatchSampler::getPatchNormal() const{
    ...
    math::Vec3f normal(a.cross(b)); // 法向量垂直于a,b
    normal.normalize();
...
void PatchSampler::computeNeighColorSamples(std::size_t v){
    ...
    // 初始化patch points在第v个视角上的颜色值
    Samples & color = neighColorSamples[v];
    // 初始化patch points在第v个视角上的像素坐标
    PixelCoords & imgPos = neighPosSamples[v];
    success[v] = false;

    /* compute pixel prints and decide on which MipMap-Level to draw the samples */
    /** 利用中心点，计算在参考帧和当前帧图像上的分辨率，从而确定相邻视角的最佳尺度 **/
    math::Vec3f const & p0 = patchPoints[nrSamples/2]; // 选择具有25个点的中间那个点的坐标
    float mfp = refV->footPrintScaled(p0); // 值越小分辨率越高
    float nfp = views[v]->footPrint(p0);
    if (mfp <= 0.f) {
    ...
    int mmLevel = 0; // mmLevel = 2，2^2=4，mmLevel是当前帧图像需要从最小的图像金字塔乘以4倍。
    while (ratio < 0.5f) { // 如果当前视角的分辨率大于参考视角，则需要进行下采样
        ++mmLevel;
        ratio *= 2.f;
    }

    /** 读取对应分辨率的当前视角V **/
    mmLevel = views[v]->clampLevel(mmLevel); // mmLvel
    core::ByteImage::ConstPtr img(views[v]->getPyramidImg(mmLevel));
    int const w = img->width(); // 当前视角的分辨率
    int const h = img->height();
    color.resize(nrSamples); // 初始化patch上的点投影到当前视图的颜色和二维坐标
    imgPos.resize(nrSamples);
