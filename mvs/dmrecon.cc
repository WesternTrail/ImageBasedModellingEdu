    ...
    , settings(_settings)
{
    ...
    refV->prepareMasterView(settings.scale); // 图片缩小4倍的结果
...
DMRecon::start()
{
    try
    {
        ...
        // 全局视角选择，主要目的是为了从参考视角的特征点找到看得到的其余的视角，并且构建图像金字塔
        globalViewSelection();

        // 处理特征，对当前的三维点投影到图像上进行深度值估计，局部选择，进一步用NCC进行过滤。
        ...
        SingleView::Ptr refV(views[settings.refViewNr]);
        if (settings.writePlyFile){ // 默认为false
            if (!settings.quiet)
    ...
    for (std::size_t i = 0; i < features.size() && !progress.cancelled; ++i){
        ...
        for (std::size_t j = 0; j < features[i].refs.size(); ++j){
            ...
            if (views[view_id]->pointInFrustum(featurePos))
                views[view_id]->addFeature(i); // 往featid里面添加当前3维坐标点的索引
        }
    }
}
...
DMRecon::globalViewSelection(){
    ...
    globalVS.performVS();
    neighViews = globalVS.getSelectedIDs();     // 此处自己的数据只有6张，全局选择最多选20张，所以一般全部选中
    ...
    for (IndexSet::const_iterator iter = neighViews.begin();
        ...
        views[*iter]->loadColorImage(0); // 读取图片金字塔,这里是从第0级开始构造
}
...
DMRecon::processFeatures(){
    ...
    for (std::size_t i = 0; i < features.size() && !progress.cancelled; ++i) { // 遍历每一个稀疏点
        /*
         * 使用当前特征如果在参考视角中可见
         * 或者在至少一个全局视角中
         */
        ...
        // 判断：1) 相机坐标系中的Z坐标是否为正； 2) 投影到图像上的坐标是否超出了图像的范围
