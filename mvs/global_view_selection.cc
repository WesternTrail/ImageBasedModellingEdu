...
GlobalViewSelection::GlobalViewSelection(
    ...
    /**如果视角没有加载，则设置成unavailable，默认全部需要加载,除了参考视角不需要去考虑 **/
...
void GlobalViewSelection::performVS(){
    ...
    selected.clear(); // 初始化已经选择了的视角列表
    bool foundOne = true;
    while (foundOne && (selected.size() < settings.globalVSMax/*最多要选择的视角个数为20个*/)){
...
GlobalViewSelection::benefitFromView(std::size_t i){
    ...
    for (std::size_t k = 0; k < nFeatIDs.size(); ++k) {
        ...
        float mfp = refV->footPrintScaled(ftPos);  // 参考视角的分辨率
        float nfp = tmpV->footPrint(ftPos); // 需要计算视角的分辨率
        float ratio = mfp / nfp;
        // reference view
        if (ratio > 2.)
            ratio = 2. / ratio;
