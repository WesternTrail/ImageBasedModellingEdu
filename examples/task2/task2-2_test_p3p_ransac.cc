...
int main(int argc, char* argv[]){
    ...
    std::ifstream fin("/home/ming/ImageBasedModellingEdu/examples/task2/correspondence2D3D.txt");
    assert(fin.is_open());
    std::string line;
    int line_id = 0;
    int n_pts = 0;
    while(getline(fin, line)){
        ...
        stream>>corr.p3d[0]>>corr.p3d[1]>>corr.p3d[2]>>corr.p2d[0]>>corr.p2d[1]; // 前3个是3d点坐标，后两个是2d点坐标
    ...
    // Ransac中止条件，内点阈判断
    sfm::RansacPoseP3P::Options pose_p3p_opts; //需要循环的次数，以及内点的阈值判断
    // Ransac估计相机姿态
    sfm::RansacPoseP3P::Result ransac_result; // 随机初始化的R,T矩阵
    sfm::RansacPoseP3P ransac(pose_p3p_opts);

    ransac.estimate(corrs, k_matrix, &ransac_result);

    std::cout<<"2D-3D correspondences inliers: "<< (100 * ransac_result.inliers.size() / corrs.size())<<std::endl; // 这里输出的是内点的比率
