...
#pragma omp parallel for
    for (std::size_t i = 0; i < observations->size(); ++i)
    {
        ...
        if (delta_x != nullptr)
        {
            ...
            update_camera(cam, delta_x->data() + cam_id, &new_camera); // 更新相机外参，内参
            ...
            update_point(p3d, delta_x->data() + pt_id, &new_point); // 更新相机的X,Y,Z坐标
            point = new_point.pos;
        }
...
void lm_optimization(std::vector<sfm::ba::Camera>*cameras
    ...
    for (int lm_iter = 0; ; ++lm_iter) {
         ...
         * 趋近于高斯牛顿法
         */
        if (successful_iteration) { // 残差值减小了，用牛顿迭代法
...
int main(int argc, char* argv[])
{
    ...
    /* 加载数据 */
    load_data("/home/ming/ImageBasedModellingEdu/examples/task2/correspondence2D3D.txt",cameras, points, observations);
