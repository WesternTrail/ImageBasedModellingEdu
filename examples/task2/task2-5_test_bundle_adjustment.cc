        ...
        , sfm::FeatureSet&feat2)
{
    ...
    for(int i=0; i<m12.size(); i++)
    {
        ...
        c2d2d.p1[0] = feat1.positions[i][0]; // 存储匹配的2d同名点
    ...
    ransac_fundamental.estimate(corr_all, &ransac_fundamental_result); // TODO 在所有同名点对用八点法来计算基础矩阵和RANSAC得到最多的内点
    // 根据RANSAC选择内点，作为匹配点。
    ...
    for(int i=0; i<ransac_fundamental_result.inliers.size(); ++i)
    {
        ...
        corr_f.push_back(corr_all[inlier_id]); // 将所有符合基础矩阵关系x2'Fx1=0的匹配点对挑出来
    }
                  ...
                  , float f2)
{
    ...
    sfm::enforce_essential_constraints(&F); // 基础矩阵的秩可能没有2，所以用奇异值分解强制执行
    ...
    sfm::pose_from_essential(E, &poses); //pose外参，

    // 利用一对匹配点从4中相机姿态中找到最合适的(利用姿态对匹配点进行三角化得到三维点，计算三维点的相机坐标，
    // 在两个相机的中的z坐标必须为正
...
main (int argc, char *argv[])
{
    ...
    // 对特征坐标进行归一化，使得求解计算稳定，RANSAC计算基础矩阵过滤sift中的外点
...
//    }
    ...
    if(!calc_cam_poses(corrs, &pose1, &pose2, f1, f2)) // 对所有內点计算一次基础矩阵和本质矩阵获得相机的参数，这里计算量会有点大，
    {
...
//    }
    ...
    std::ofstream out("/home/ming/ImageBasedModellingEdu/examples/task2/test_ba.txt");
    assert(out.is_open());

    /*捆绑调整*/
    std::vector<sfm::ba::Camera> cams(2);  // 初始化2个相机
    ...
    for(int i=0; i<observations.size(); i++){  // 打印观察点对应的3d点索引，以及观察点的二维坐标
