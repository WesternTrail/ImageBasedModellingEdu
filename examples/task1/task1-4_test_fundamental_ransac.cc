...
double calc_sampson_distance(FundamentalMatrix const& F, sfm::Correspondence2D2D const& m) {
    ...
    double sum = 0.0;
    sum += math::fastpow(m.p1[0] * F[0] + m.p1[1] * F[1] + F[2], 2); // 注意矩阵的下表
...
void calc_fundamental_least_squares(sfm::Correspondences2D2D const & matches, FundamentalMatrix&F){
    ...
    math::matrix_svd<double>(&A[0], matches.size(), 9, nullptr, nullptr, &vv[0]); //可能是取第0个，后面还会选取9个组成矩阵？
...
int main(int argc, char *argv[]){
    ...
    std::ifstream in("/home/ming/ImageBasedModellingEdu/examples/task1/correspondences.txt");
    assert(in.is_open());
    ...
    std::cout << "RANSAC-F: Running for " << n_iterations
              ...
              << "..." << std::endl;
    for(int i=0; i<n_iterations; i++){ // 进行1177次循环
        ...
        for(int j=0; j<8; j++, iter++){ // 读取8对点的坐标计算基础矩阵
        ...
        if(inlier_indices.size()> best_inliers.size()){ //如果这次内点数目更多
