...
public:
    ...
    /**
        ...
        p: 返回的像素坐标系的点坐标
    **/
        math::Vec2d p; // 就两个坐标u,v
        double p_cx = R_[0] * p3d[0] + R_[1] * p3d[1] + R_[2] * p3d[2] + t_[0];
        double p_cy = R_[3] * p3d[0] + R_[4] * p3d[1] + R_[5] * p3d[2] + t_[1];
        double p_cz = R_[6] * p3d[0] + R_[7] * p3d[1] + R_[8] * p3d[2] + t_[2];

        double x = double(p_cx) / p_cz; // 转换到归一化像平面
        double y = double(p_cy) / p_cz;

        double dist = x*x + y*y;
        double ratio = 1 + dist_[0] * dist + dist_[1] * dist * dist;

        p[0] = f_ * (ratio * x) + c_[0]; //归一化像平面->物理像平面->像素像平面
        p[1] = f_ * (ratio * y) + c_[1];
        return p;
