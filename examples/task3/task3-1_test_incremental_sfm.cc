...
features_and_matching (core::Scene::Ptr scene,
    ...
    /* viewports：包括了focal，pose,image,features，track_ids等信息
     * pairwise_matching：有n(n-1)/2个元素，每个元素包含：view1_id,view2_id，matches匹配的特征点对
     * */
    ...
    std::cout << "Computing image features..." << std::endl;
    {
        ...
        bundler_features.compute(scene, viewports); //计算每一张图片的特征
    ...
    std::cout << "Performing feature matching..." << std::endl;
    {
        ...
        bundler_matching.init(viewports);  // 视图的参数初始化
        bundler_matching.compute(pairwise_matching); // 构建图像连接图，并且匹配特征点构成matches放在pairwise_matching中
...
int main(int argc, char *argv[])
{
    ...
    core::Scene::Ptr scene = make_scene(argv[1], argv[2]); // 从可交换文件中导出exif，resize后的图片，保存在output文件夹的views目录
    std::cout<<"Scene has "<<scene->get_views().size()<<" views. "<<std::endl;


    /* 进行特征匹配 */
    sfm::bundler::ViewportList viewports; // 视图信息同时也包括了tracks_id
    sfm::bundler::PairwiseMatching pairwise_matching; // 特征匹配的点对索引
    ...
    sfm::bundler::InitialPair::Options init_pair_opts;
        ...
        init_pair.compute_pair(&init_pair_result); // 选择初始的一对相机
    ...
    incremental_opts.pose_p3p_opts.max_iterations = 1000; // 1000次迭代？
    ...
    incremental_opts.min_triangulation_angle = MATH_DEG2RAD(1.0); // 最小的量测角
    incremental_opts.ba_fixed_intrinsics = false; //是否固定内参？
    //incremental_opts.ba_shared_intrinsics = conf.shared_intrinsics;
    incremental_opts.verbose_output = true; // 打印BA信息？
    ...
    incremental.bundle_adjustment_full(); // 全局BA调整
    ...
    while (true)
    {
        ...
        if (full_ba_num_skipped < full_ba_skip_views)
        {
            ...
            incremental.triangulate_new_tracks(MIN_VIEWS_PER_TRACK); // 三角量测
            std::cout << "Running full bundle adjustment..." << std::endl;
    ...
    for(int i=0; i<tracks.size(); i++){ // 将所有有效的track添加
    ...
    for(int i=0; i< valid_tracks.size(); i++){ // 写入tracks内容
