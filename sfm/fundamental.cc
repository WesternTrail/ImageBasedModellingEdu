    ...
    FundamentalMatrix* result)
{
    ...
    std::vector<double> A(points.size() * 9); //不单单是8点，所有内点一起计算基础矩阵
    ...
    Eight2DPoints const& points_view_2, FundamentalMatrix* result)
{
    ...
    for (int i = 0; i < 8; ++i)
    {
        ...
        A(i, 0) = p2[0] * p1[0]; // u2u1
        A(i, 1) = p2[0] * p1[1]; // u2v1
        A(i, 2) = p2[0] * 1.0; // u2
        A(i, 3) = p2[1] * p1[0]; // v2u1
        A(i, 4) = p2[1] * p1[1]; // v2v2
        A(i, 5) = p2[1] * 1.0; // v2
        A(i, 6) = 1.0   * p1[0]; // u1
        A(i, 7) = 1.0   * p1[1]; // v1
        A(i, 8) = 1.0   * 1.0; // 1
...
{
    /* 八点法得到的基础矩阵可能不满足秩为2，自由度为8的要求，
     * 所以要先对matrix进行奇异值分解，matrix = USVt,S = diag(θ1，θ2，θ3)
     * 再直接令S = diag(θ1，θ2，0)，重新计算US'Vt即为F
     * */
