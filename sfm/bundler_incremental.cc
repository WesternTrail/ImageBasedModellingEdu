    ...
    SurveyPointList* survey_points)
{
    ...
    for (std::size_t i = 0; i < tracks->size(); ++i)
    {
        ...
        track.invalidate(); // 将所有tracks对应三维空间点的坐标设置为nan
    }
}
...
Incremental::find_next_views (std::vector<int>* next_views)
{
    ...
    for (std::size_t i = 0; i < this->tracks->size(); ++i) // 找到能看到最多的点的那个视图
    {
        Track const& track = this->tracks->at(i);
        if (!track.is_valid()) // track对应的三维坐标是否是有效值
            continue;

        for (std::size_t j = 0; j < track.features.size(); ++j)
        {
            int const view_id = track.features[j].view_id;
            if (this->viewports->at(view_id).pose.is_valid()) // 找到一个特征点对应的视图没有进行BA的，判断标准是内参矩阵第一个元素是否为0
                continue;
            valid_tracks[view_id].first += 1; //view_id这里一般是未进行ba调整的视图索引，first代表特征点的个数
        }
    }
...
Incremental::triangulate_new_tracks (int min_num_views)
{
    ...
    for (std::size_t i = 0; i < this->tracks->size(); ++i){ // 遍历每一对tracks
        ...
        for (std::size_t j = 0; j < track.features.size(); ++j){ // 遍历track中的所有特征点
...
Incremental::bundle_adjustment_intern (int single_camera_ba){
    ...
    for (std::size_t i = 0; i < this->tracks->size(); ++i) //tracks->size： 5176
    {
        Track const& track = this->tracks->at(i);
        if (!track.is_valid())
            continue;
    ...
    for (std::size_t i = 0; i < this->viewports->size(); ++i)
    {
        if (ba_cameras_mapping[i] == -1) // 如果遍历到了最后一张视图，则停止
            continue;
...
Incremental::invalidate_large_error_tracks (void)
{
    ...
    for (std::size_t i = 0; i < this->tracks->size(); ++i){
        ...
        for (std::size_t j = 0; j < ref.size(); ++j){
            ...
            x2d *= (1.0 + r2 * (viewport.radial_distortion[0] + viewport.radial_distortion[1] * r2)) * pose.get_focal_length(); // 乘以k和畸变系数得到物理像平面的坐标
