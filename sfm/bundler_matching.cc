...
Matching::compute (PairwiseMatching* pairwise_matching)
{
    ...
    std::size_t num_pairs = num_viewports * (num_viewports - 1) / 2; // 两两配对后的pair数量
...
#pragma omp critical
        ...
        std::swap(matching.matches, matches); // 将matches赋值给matching.matches
...
{   // matches：最终用ransac计算基础矩阵过滤掉外点匹配点的索引对
    FeatureSet const& view_1 = this->viewports->at(view_1_id).features;
    FeatureSet const& view_2 = this->viewports->at(view_2_id).features;
    ...
    sfm::Correspondences2D2D unfiltered_matches; // 存储匹配点的坐标
    sfm::CorrespondenceIndices unfiltered_indices; // 存储特征匹配点的索引
    {
        std::vector<int> const& m12 = matching_result.matches_1_2;
        for (std::size_t i = 0; i < m12.size(); ++i)
        {
            if (m12[i] < 0) // m12 < 0 的就是没匹配到点的
                continue;
    ...
    sfm::RansacFundamental::Result ransac_result; // 存储是内殿的
    int num_inliers = 0;
    {
