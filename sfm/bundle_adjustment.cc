...
BundleAdjustment::lm_optimize (void)
{
    ...
    double current_mse = this->compute_mse(F);// F：512x2，2是代表u,v轴的残差,共有512个观察点，256对匹配点
    ...
    for (int lm_iter = 0; ; ++lm_iter)
    {
        if (lm_iter + 1 > this->opts.lm_min_iterations
            && (current_mse < this->opts.lm_mse_threshold))  // 当前计算的损失小于我们设定的阈值，停止迭代
        {
        ...
        {
            /*重新计算相机和三维点，计算重投影误差，注意原始的相机参数还没有被更新*/
        ...
        if (successful_iteration)
        {
            if (delta_mse_ratio < this->opts.lm_delta_threshold) //新的mse与旧的mse相比变化不大时，同样停止迭代
            {
    ...
    SparseMatrixType* jac_points)
{
    ...
    std::size_t const camera_cols = this->cameras->size() * this->num_cam_params; // num_cam_params不包括f,k0,k1所以为6
