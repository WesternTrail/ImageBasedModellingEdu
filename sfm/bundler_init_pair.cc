...
InitialPair::compute_pair (Result* result)
{
    ...
    /*
     * 找到最好的一对相机对，（满足匹配的特征点足够多，基线足够长）
     * 如果没有找到合适的，返回最大分数的那一个
     */
    bool found_pair = false;std::size_t found_pair_id = std::numeric_limits<std::size_t>::max();
...
#pragma omp parallel for schedule(dynamic)
    for (std::size_t i = 0; i < candidates.size(); ++i)
    {
        ...
        pair_scores[i] = this->score_for_pair(candidate, num_inliers, angle); // 根据内点数量，测量角计算得分
...
InitialPair::compute_candidate_pairs (CandidatePairs* candidates)
{
    ...
    for (std::size_t i = 0; i < this->tracks->size(); ++i) // 遍历track的数量
    {
        Track const& track = this->tracks->at(i);
        for (std::size_t j = 1; j < track.features.size(); ++j) // 遍历当前track横跨的图片张数？
            for (std::size_t k = 0; k < j; ++k) { // 0-1 0-2 1-2三种

                int v1id = track.features[j].view_id; // track对应第一张视图的索引
                int v2id = track.features[k].view_id; // track对应第二章视图的索引
                int f1id = track.features[j].feature_id; // track对应第一个特征点的索引
                int f2id = track.features[k].feature_id; // track对应第二个特征点的索引
                if (v1id > v2id) { //
                    std::swap(v1id, v2id);
                    std::swap(f1id, f2id);
                }
                ...
                /* 查找pair */
                int const lookup_id = v1id * num_viewports + v2id; // 类似于构造哈希键？
                int pair_id = candidate_lookup[lookup_id];
                if (pair_id == -1) // 如果是-1则创建pair加入
                {
                ...
                candidates->at(pair_id).matches.push_back(match); // 添加match信息
            }
    }
}
    ...
    CameraPose* pose1, CameraPose* pose2)
{
    ...
    {
        poses[i].K = pose2->K;
        if (is_consistent_pose(candidate.matches[0], *pose1, poses[i])) // 用第一对匹配点验证
        {
            *pose2 = poses[i];
            found_pose = true;
            break;
    ...
    CameraPose const& pose1, CameraPose const& pose2)
{
    ...
    for (std::size_t i = 0; i < candidate.matches.size(); ++i)
    {
        ...
        p1 = T1.mult(p1).normalized(); // 从图像坐标系到世界坐标系得到射线坐标
