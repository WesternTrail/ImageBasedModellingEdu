...
void
RansacFundamental::estimate (Correspondences2D2D const& matches, Result* result) //matches：sift匹配的点对
{
    ...
    for (int iteration = 0; iteration < this->opts.max_iterations; ++iteration)
    {
        ...
        this->estimate_8_point(matches, &fundamental);  // 8点法求解基础矩阵

        this->find_inliers(matches, fundamental, &inliers); // 计算内点个数
        if (inliers.size() > result->inliers.size())
        {
            ...
            inliers.reserve(matches.size());  //为vector主动分配内存空间，并不需要添加一次，分配一次
        }
    }
}
    ...
    FundamentalMatrix const& fundamental, std::vector<int>* result)
{
    ...
    {
        double error = sampson_distance(fundamental, matches[i]); // 计算距离
