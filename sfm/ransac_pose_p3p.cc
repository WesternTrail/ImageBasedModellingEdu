...
#pragma omp for
        for (int i = 0; i < this->opts.max_iterations; ++i)
        {
            ...
            PutativePoses poses;
            this->compute_p3p(corresp, inv_k_matrix, &poses); // 从随机选的3对点中得到4组解
...
#pragma omp critical
                if (inliers.size() > result->inliers.size()){
                    result->pose = poses[j]; // 这里通过最多的内点判断解的有效性
                    std::swap(result->inliers, inliers);
                    inliers.reserve(corresp.size()); // 用于反转数据
...
RansacPoseP3P::compute_p3p (Correspondences2D3D const& corresp,
    ...
    /* corresp: 3d-2d点对
     * inv_k_matrix: 从像素坐标系到归一化像平面坐标系的投影矩阵
     * poses: 待求解的[R,t]矩阵
     * */
    ...
    Pose const& pose, std::vector<int>* inliers) const
{
    ...
    for (std::size_t i = 0; i < corresp.size(); ++i)
    {
        ...
        math::Vec3d p2d = k_matrix * (pose * p3d); // 世界坐标系转到相机坐标系
